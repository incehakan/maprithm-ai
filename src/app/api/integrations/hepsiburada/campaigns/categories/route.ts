...
import { getHbCampaignEligibleCategories } from "@/lib/hepsiburadaCampaigns";
...
export async function GET() {
  ...
  try {
    const result = await getHbCampaignEligibleCategories(ctx.storeId);
    if (!result.ok) {
    ...
    console.error("HB campaigns categories GET error:", error);
    return createErrorResponse(error, {
      route: "GET /api/integrations/hepsiburada/campaigns/categories",
    });
  }
}
