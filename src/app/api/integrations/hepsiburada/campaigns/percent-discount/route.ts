...
import {
  createHbPercentDiscount,
  type HbPercentDiscountRequest,
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json().catch(() => null)) as HbPercentDiscountRequest | null;
    ...
    const result = await createHbPercentDiscount(ctx.storeId, body);
    if (!result.ok) {
    ...
    console.error("HB campaigns percent-discount POST error:", error);
    return createErrorResponse(error, {
      route: "POST /api/integrations/hepsiburada/campaigns/percent-discount",
    });
  }
}
