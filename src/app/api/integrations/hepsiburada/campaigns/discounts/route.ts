...
import { getHbCampaignDiscounts } from "@/lib/hepsiburadaCampaigns";
...
export async function GET() {
  ...
  try {
    const result = await getHbCampaignDiscounts(ctx.storeId);
    if (!result.ok) {
    ...
    console.error("HB campaigns discounts GET error:", error);
    return createErrorResponse(error, {
      route: "GET /api/integrations/hepsiburada/campaigns/discounts",
    });
  }
}
