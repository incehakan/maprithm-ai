...
import { createHbTlDiscount, type HbTlDiscountRequest } from "@/lib/hepsiburadaCampaigns";
...
export async function POST(request: Request) {
  ...
  try {
    const body = (await request.json().catch(() => null)) as HbTlDiscountRequest | null;
    ...
    const result = await createHbTlDiscount(ctx.storeId, body);
    if (!result.ok) {
    ...
    console.error("HB campaigns tl-discount POST error:", error);
    return createErrorResponse(error, {
      route: "POST /api/integrations/hepsiburada/campaigns/tl-discount",
    });
  }
}
