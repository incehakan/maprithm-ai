...
import { getHbCampaignBudgets } from "@/lib/hepsiburadaCampaigns";
...
export async function GET() {
  ...
  try {
    const result = await getHbCampaignBudgets(ctx.storeId);
    if (!result.ok) {
    ...
    console.error("HB campaigns budgets GET error:", error);
    return createErrorResponse(error, {
      route: "GET /api/integrations/hepsiburada/campaigns/budgets",
    });
  }
}
