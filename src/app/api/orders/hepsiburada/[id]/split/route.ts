/**
 * POST /api/orders/hepsiburada/[id]/split
 * splitHbPackage — body: `{ lineItems: string[] }`
...
import { splitHbPackage, type HbSplitPackageBody } from "@/lib/hepsiburadaPackageOps";
...
export async function POST(req: Request, { params }: Params) {
  ...
  const lineItemsRaw = raw.lineItems ?? raw.lineItemIds;
  const lineItems = Array.isArray(lineItemsRaw)
    ? lineItemsRaw.map((x) => String(x)).filter(Boolean)
    : [];
  if (lineItems.length === 0) {
    return NextResponse.json(
      { success: false, error: "body.lineItems (string[]) zorunludur." },
      { status: 400 }
    );
  }
  const body: HbSplitPackageBody = { lineItems };
  ...
  const result = await splitHbPackage({
    storeId: ctx.storeId,
    packageNumber,
    body,
  });
